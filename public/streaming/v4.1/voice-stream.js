...
//#CallInitialize p1
var socketId='',myStream='',screen='',pc=[],PwcUIKitPrebuilt={},PwcFunctionality={},SOCKET='';
var ROOM='',USER_ID='',USER_NAME='',CALL_ID='', mainstreaming="", chatroomUsers={},turnOnMicrophoneWhenJoining=true,entSalt = "enc"+Date.now();
const baseHtmlCode = '<div class="row"><audio class="local-video mirror-mode" id="local" volume="0" autoplay muted></audio></div><div class="row"><div class="col-md-12 main" id="main-section"> <div class="row mt-2 mb-2" id="videos"></div></div></div>';

PwcUIKitPrebuilt.generateKitToken = (roomID, userID, userName) => {
    var dataObj = { roomID, userID, userName };
    console.log("generateKitToken", dataObj);
    var encrypted = encryptData(dataObj, entSalt);
    return encrypted;
};

PwcUIKitPrebuilt.create = (kitToken) =>{
    console.log("create", kitToken);
    var decryptedData = decryptData(kitToken, entSalt);
    if (decryptedData && decryptedData.roomID && decryptedData.userID && decryptedData.userName) {
        ROOM=decryptedData.roomID;
        USER_ID=decryptedData.userID;
        USER_NAME=decryptedData.userName;
        return PwcFunctionality;
    } else {
        throw new Error('Invalid Kit Token');
    }
};

// PwcUIKitPrebuilt.create = (roomID, userID, userName) =>{
//     ROOM=roomID;USER_ID=userID;USER_NAME=userName;
//     return PwcFunctionality;
// };

PwcFunctionality.joinRoom=(dataObj)=>{
    if (ROOM==''||USER_ID==''||USER_NAME=='') return;
    console.log("joinRoom", dataObj);
    const container=dataObj.container;
    mainstreaming = container.id;
    document.getElementById(mainstreaming).innerHTML+=baseHtmlCode;
    turnOnMicrophoneWhenJoining = (dataObj.turnOnMicrophoneWhenJoining) ? dataObj.turnOnMicrophoneWhenJoining:false;
    initializeCallSystem();
    dbStoreCallStart();
};

PwcFunctionality.mute = () => {
    if (myStream) {
...
PwcFunctionality.userCount = () => {
    let elem = document.getElementsByClassName( 'card' );
    return elem.length;
};

PwcFunctionality.userList = () => {
    return chatroomUsers;
};

//#CallInitialize p4
PwcFunctionality.endCall=()=>{
    SOCKET.emit('leave call',{});
    for(let dataId of pc){closeVideo(dataId);}
    document.getElementById(mainstreaming).innerHTML="";
    dbStoreCallEnd();
};
...
const getIceServer = () => {
    return {
        iceServers: [
            {
                urls: ["stun:20.204.112.141:3478"]
            },
            {
                username: "turnserver",
                credential: "Himakshi$#9900",
                urls: [
                    "turn:20.204.112.141:3478",
                    "turn:20.204.112.141:3478?transport=udp",
                    "turn:20.204.112.141:3478?transport=tcp"
                ]
            }
        ]
    };
...
function encryptData(o, salt) {
    o = JSON.stringify(o).split('');
    for(var i = 0, l = o.length; i < l; i++){
        if(o[i] == '{') o[i] = '}';
        else if(o[i] == '}') o[i] = '{';
    }
    let encodeURIData = encodeURI(salt + o.join(''));
    encodeURIData = encodeURIData.replaceAll("A", "#jh63#");
    encodeURIData = encodeURIData.replaceAll("a", "#84ht#");
    encodeURIData = encodeURIData.replaceAll("E", "#h93b#");
    encodeURIData = encodeURIData.replaceAll("i", "#8n2r#");
    encodeURIData = encodeURIData.replaceAll("O", "#k347#");
    encodeURIData = encodeURIData.replaceAll("o", "#j64l#");
    encodeURIData = encodeURIData.replaceAll("U", "#92hd#");
    encodeURIData = encodeURIData.replaceAll("u", "#pw93#");
    encodeURIData = encodeURIData.replaceAll("e", "#c46h#");
    encodeURIData = encodeURIData.replaceAll("I", "#7lw4#");
    return encodeURIData;
};

function decryptData(o, salt) {
    try {
        o = o.replaceAll("#jh63#", "A");
        o = o.replaceAll("#84ht#", "a");
        o = o.replaceAll("#h93b#", "E");
        o = o.replaceAll("#8n2r#", "i");
        o = o.replaceAll("#k347#", "O");
        o = o.replaceAll("#j64l#", "o");
        o = o.replaceAll("#92hd#", "U");
        o = o.replaceAll("#pw93#", "u");
        o = o.replaceAll("#c46h#", "e");
        o = o.replaceAll("#7lw4#", "I");
        o = decodeURI(o);
...
function initializeCallSystem() {
    if (!ROOM) {
        alert("please enter room id to inintialize call");
    } else {
        getAndSetUserStream();
        ...
        SOCKET.on('connect', function(){
            ...
            SOCKET.emit('room join',{socketId:socketId, room:ROOM, userId:USER_ID, userName:USER_NAME});

            SOCKET.on('new joined', function(data) {
                SOCKET.emit('old user', { to: data.socketId, sender: socketId, userId:USER_ID, userName:USER_NAME });
                pc.push(data.socketId);
                chatroomUsers[data.socketId] = {};
                chatroomUsers[data.socketId]["userId"] = data.userId;
                chatroomUsers[data.socketId]["userName"] = data.userName;
                init(true, data.socketId);
            });
            SOCKET.on('old user', function( data ) {
                pc.push(data.sender);
                chatroomUsers[data.sender] = {};
                chatroomUsers[data.sender]["userId"] = data.userId;
                chatroomUsers[data.sender]["userName"] = data.userName;
                init(false, data.sender);
            });

            SOCKET.on('ice candidates', async function( data ) {
                data.candidate ? await pc[data.sender].addIceCandidate(new RTCIceCandidate( data.candidate )) : '';
            });
            SOCKET.on('pwc', async function( data ) {
                if ( data.description.type === 'offer' ) {
                    data.description ? await pc[data.sender].setRemoteDescription( new RTCSessionDescription( data.description ) ) : '';
                    getUserFullMedia().then( async function( stream ) {
                        // if ( !document.getElementById( 'local' ).srcObject ) {
                        //     //setLocalStream( stream ); //last connected users local stream
                        // }
                        //save my stream
                        myStream = stream;
                        ...
                        SOCKET.emit( 'pwc', { description: pc[data.sender].localDescription, to: data.sender, sender: socketId } );
                    } ).catch( function( e ) {
                        console.error( e );
                    } );
            ...
            SOCKET.on('leave call', function( data ) {
                delete chatroomUsers[data.socketId];
                closeVideo(data.socketId);
            });
        });
        function init(createOffer, partnerName) {
            ...
            if (screen && screen.getTracks().length) {
                ...
                if (!turnOnMicrophoneWhenJoining) PwcFunctionality.mute();
                myStream.getTracks().forEach( function( track ) {
                    pc[partnerName].addTrack( track, myStream );//should trigger negotiationneeded event
                });
            }
            else {
                getUserFullMedia().then( function( stream ) {
                    //save my stream
                    myStream = stream;
                    if (!turnOnMicrophoneWhenJoining) PwcFunctionality.mute();
                    stream.getTracks().forEach( function( track ) {
            ...
            //create offer
            if (createOffer) {
                pc[partnerName].onnegotiationneeded = async function() {
                    let offer = await pc[partnerName].createOffer();
                    await pc[partnerName].setLocalDescription( offer );
                    SOCKET.emit('pwc', { description: pc[partnerName].localDescription, to: partnerName, sender: socketId } );
                };
            }
            //send ice candidate to partnerNames
            pc[partnerName].onicecandidate = function( { candidate } ){
                SOCKET.emit('ice candidates', { candidate: candidate, to: partnerName, sender: socketId } );
            };
            //add
            pc[partnerName].ontrack = function( e ) {
                let str = e.streams[0];
            ...
            pc[partnerName].onconnectionstatechange = function( d ) {
            ...
            pc[partnerName].onsignalingstatechange = function( d ) {
...
//#CallInitialize p5
function dbStoreCallStart(){
    if (ROOM==''||USER_ID==''||USER_NAME=='') return;
    const myBody = {ROOM,USER_ID,USER_NAME};
    console.log("dbStore CallStart", myBody);
...
function dbStoreCallEnd(){
    if (ROOM==''||USER_ID==''||USER_NAME=='') return;
    const myBody = {ROOM,USER_ID,USER_NAME};
    console.log("dbStore CallEnd", myBody);
    fetch('/api/call-end', {
        method: 'POST',
        body: JSON.stringify(myBody),
        headers: {
...
PwcFunctionality.getCallHistory=()=>{
    return  new  Promise((resolve,reject)=>{
        fetch('/api/call-history?roomId='+ROOM)
